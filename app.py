...
# Data Visualization Page
    ...
    try:
        ...
        if run_choices:
            ...
            st.write(selected_run_data.select_dtypes(include=['number'], exclude=['datetime']).describe())

            # Optional charts
            if st.checkbox("Show charts"):
                # Columns to exclude from visualization
                cant_use = ['Well', 'Culture Time (h)', 'IPTG (mM)', 'Bioreactor']
                numeric_columns = selected_run_data.select_dtypes(include=np.number).columns
                filtered_columns = [col for col in numeric_columns if col not in cant_use]
    ...
    st.cache_data.clear()
    st.header("KPI and Initial Conditions Visualization")
    ...
    try:
        ...
        if df_choice == "1mL":
            ...
            row_map = {'A': 0, 'B': 1, 'C': 2, 'D': 3, 'E': 4, 'F': 5, 'G': 6, 'H': 7} # Extend if you have more rows
            col_map = {str(i): i - 1 for i in range(1, 13)} # Adjust range if you have more columns
            ...
            heatmap_df['Well_Index'] = heatmap_df['Well'].astype(str).map(col_map)

            # Drop rows with NaN in the index columns if your original data doesn't perfectly fill the plate
            heatmap_df = heatmap_df.dropna(subset=['Row_Index', 'Well_Index'])
            ...
            fig = px.imshow(heatmap_df.pivot_table(index='Row_Index', columns='Well_Index', values=color_column),
                            labels=dict(x="Well", y="Row", color=color_column),
                            x=[str(i) for i in range(1, 9)], # Adjust range based on your 'Well' numbers
                            y=list(row_map.keys())[:6], # Adjust range based on your 'Row' letters
            ...
            fig.update_traces(hovertemplate="<b>Row:</b> %{y}<br><b>Well:</b> %{x}<br><b>Value</b>: %{z}<extra></extra>")
            ...
            # Define a layout
            n_cols = 3
            n_rows = (n_bioreactors + n_cols - 1) // n_cols  # Calculate number of rows needed  

            temp = df.dropna(axis=1, how='all').drop(columns=['Bioreactor'])
            numerical_cols = temp.select_dtypes(include=['number']).columns.tolist()
            metric_to_visualize = st.selectbox("Select metric to visualize:", numerical_cols)
            ...
            fallback_viridis = ["#440154", "#482878", "#3e4a89", "#31688e", "#26828e", "#1f9e89", "#35b779", "#6ece58", "#b5dc36", "#fde725"]
            ...
            for i, bioreactor in enumerate(bioreactors):
                ...
                if max_val - min_val == 0:
                    ...
                    normalized_value = (avg_metric - min_val) / (max_val - min_val)

                # Get color from the fallback list
                color_index = int(normalized_value * (len(fallback_viridis) - 1))
                color_val = fallback_viridis[color_index]

                # Add colored background
                fig.add_trace(go.Scatter(x=[0.5], y=[0.5],
                                        mode='markers',
                                        marker=dict(size=50, color=color_val),
                                        showlegend=False),
                ...
                fig.update_xaxes(visible=False, range=[0, 1], row=row_idx, col=col_idx)
                fig.update_yaxes(visible=False, range=[0, 1], row=row_idx, col=col_idx)
            ...
            fig.update_layout(title_text=f"Average {metric_to_visualize} by Bioreactor (Darker = Less)")
            st.plotly_chart(fig)
